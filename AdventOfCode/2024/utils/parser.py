...
def chunked_lines(
    file_name: str = "", path: str = "", contents: str = "", chunk_size: int = 0
) -> list[list[str]]:
    lines = None
    if contents:
...
"""
A function that creates a grid using the delimiter if one is given, or each character
Fill out the 'contents' variable with the contents of your input if you wish to add them manually
Fill out the 'path' and 'file_name' variables with the specified path if your input lies elsewhere. Default is cwd/inputs/file_name 
"""
...
def get_grid(
    file_name: str = "", path: str = "", contents: str = "", delimiter: str = ""
) -> list[list[str]]:
    lines = None
    if contents:
        lines = contents.strip().split("\n")
    ...
    ret = []
    if delimiter == "":
        ret = [[c for c in line] for line in lines]
    else:
